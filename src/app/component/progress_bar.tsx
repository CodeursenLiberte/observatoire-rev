...
const Part = ({props, status, left, right}: {props: Props, status: TronçonStatus, left?: boolean, right?: boolean}) => {
  ...
  return <div style={{width: `${width}%`, background: statusColor[status]}} className={className} />
}

export default function ProgressBar(props: Props) {
  const part_ok = 100 * (props.stats[TronçonStatus.PreExisting] + props.stats[TronçonStatus.Built] + props.stats[TronçonStatus.Building]) / props.total
  return <div className="pb-4">
    ...
    <div className="progress-bar-percent is-size-4 has-text-weight-bold has-text-centered" style={{marginLeft: `${part_ok}%`}}>{Math.round(part_ok)}%</div>
  </div>
}
